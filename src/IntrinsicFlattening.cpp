...
Eigen::VectorXd IntrinsicFlattening::QPSolve(
    ...
    Eigen::VectorXd x, y, z;
    // DEBUG
    // cout << A.isApprox(A.triangularView<Eigen::Lower>(),0) << "WASSIM" <<
    // endl;
    /*
    cout << C << endl;
    cout << d << endl;
...
template <typename... Args> Eigen::VectorXd concat(size_t size, Args &...args) {
...
IntrinsicFlattening::VertexAngleSumConstraint(VertexData<double> curvatures) {
    // =========== Equation [3]
    // ================================================= For each interior
    // vertex, add a constraint that says the angles around the vertex sum to 2π
    // - Ω_i:
    //
    //     _______
    //    /\     /\
...
pair<vector<T>, vector<double>> IntrinsicFlattening::EdgeDelaunayConstraint() {
    ...
    // add a constraint which ensures that the two opposite angles in the CAS
    // satisfy the Delaunay condition , namely
    //
    //   α0 + α1 < π       [4]
    //
...
// |E| x 2 |C|
pair<vector<T>, vector<double>>
IntrinsicFlattening::EdgeIntersectionAngleConstraint() {
    // =========== Equation [1]
    // ================================================= For each interior edge,
...
//|C| x |C|
pair<vector<T>, vector<double>>
IntrinsicFlattening::AngleDeviationPenalty(CornerData<double> beta) {
    vector<T> tripletList;
