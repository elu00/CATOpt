...
// outputs the current CAT triangulation described by flattened and alphas to
// the desired filename
void CatOpt::dbgSVG(string filename) {
    std::ofstream ss(filename, std::ofstream::out);  // For filesystem I/O
    // SVG header stuff
    ...
    for (Vertex v : mesh->vertices()) {
        ...
        ss << "<circle cx=\"" << shift(thing.x) << "\" cy=\"" << shift(thing.y)
           << "\" r=\"1\"/>" << endl;
    }
    ...
    for (Edge e : mesh->edges()) {
        ...
        if (abs(angle) > 1e-7) {  // the edge is an actual arc
            string largeArcFlag =
            ...
            ss << "<path d=\"M" << shift(i.x) << "," << shift(i.y) << " A"
               ...
               << endl;
        } else {  // if the line's flat, just output a line
...
double CatOpt::confObjective(vector<Vector2>& flattened, vector<double>& alphas) {
    ...
    for (Vertex v : mesh->vertices()) {
        ...
        if (first) {
            if (v.isBoundary()) {
                // cout << "Actual boundary sum: " << accum << "target: " <<
                // targetAccum << endl;
            }
            first = false;
        }
    ...
    for (Corner C : mesh->corners()) {
        ...
        if (h.isInterior()) {
            ...
            // cout << "target:" << targetAngles[C] << "   " << "actual:" <<
            // angle << endl;
            // check: sign issue?
            if (ab.vertex().isBoundary()) {
                // cout << "BOUNDARY " << angle << " target " << targetAngles[C]
                // << endl;
            }
...
void CatOpt::confGradient(vector<Vector2>& x, vector<double>& alphas,
...
// \[ E(p,\alpha) = \sum_{i^{jk}} (\beta_i^{jk} - \tilde{\theta}_i^{jk}(p) +
// \tilde{\alpha}_{ij} + \tilde{\alpha}_{ik})^2 \] that measures the distortion
// of the current corner angles \(\tilde{\beta}\), relative to the desired
// corner angles \(\beta\).
void CatOpt::confStep(int nDescentSteps) {
    cout << "Testing derivatives for planar optimization..." << endl;
    testFlatteningDerivatives();

    cout << "Taking up to " << nDescentSteps
    ...
    // DEBUG: plot the convergence rate of the first iterations
    std::ofstream grad("gradient", std::ofstream::out);
    std::ofstream obj("objective", std::ofstream::out);
    ...
    for (int descentStep = 0; descentStep < nDescentSteps; descentStep++) {
        ...
        // compute the squared norm of the gradient at the previous
        // configuration
        normGrad2 = 0.;
        ...
        for (lineSearchStep = 0; lineSearchStep < maxLineSearchSteps;
            ...
            // cout << "Product:" << lineSearchAlpha * t * normGrad2 << endl;
            if (confObjective(p, alpha) < E - lineSearchAlpha * t * normGrad2) {
                break;
            }
        ...
        // replace old configuration (p0,alpha0) with new configuration
        // (p,alpha)
        p0 = p;
        alpha0 = alpha;
        ...
        if (normGrad2 < 1e-10 || descentStep == nDescentSteps - 1) {
            cout << "Iterations Taken:" << descentStep << endl;
            cout << "energy:" << E << endl;
            cout << "gradient norm" << sqrt(normGrad2) << endl;
            dbgSVG("final_flattened.svg");
            break;
        }
    }
}
...
void CatOpt::testFlatteningDerivatives() {
    ...
    for (int v = 0; v < nVertices; v++) {
        Vector2
            dEdpv;  // numerical derivative with respect to position of vertex v
...
void CatOpt::loadModel(const std::string& inputPath, bff::Model& model,
    ...
    if (bff::MeshIO::read(inputPath, model, error)) {
        ...
        std::cerr << "Unable to load file: " << inputPath << ". " << error
                  << std::endl;
        exit(EXIT_FAILURE);
    }
}
...
// Code from BFF
void CatOpt::flatten(bff::Model& model, const std::vector<bool>& surfaceIsClosed, int nCones, bool flattenToDisk, bool mapToSphere) {
    bff::Mesh& mesh = model[0];
    bff::BFF bff(mesh);
    ...
    if (nCones > 0) {
        ...
        if (bff::ConePlacement::findConesAndPrescribeAngles(
                S, cones, coneAngles, bff.data, mesh) ==
        ...
        if (surfaceIsClosed[0]) {
            std::cerr
                << "Surface is closed. Either specify nCones or mapToSphere."
                << std::endl;
            exit(EXIT_FAILURE);
        } else {
...
// Calls BFF, then computers target angles and calls optimization procedure.
void CatOpt::conformalFlatten() {
    ...
    for (bff::VertexCIter v = model[0].vertices.begin();
         v != model[0].vertices.end(); v++) {
...
void CatOpt::testSVG() {
    ...
    ss << "<?xml version=\"1.0\" standalone=\"no\" ?>" << endl
       ...
       << "<svg width=\"2000\" height=\"2000\" "
          "xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" >"
       << endl;
    ss << "<circle cx=\"" << 500 << "\" cy=\"" << (500) << "\" r=\"1\"/>"
       << endl;
    ss << "<circle cx=\"" << 500 << "\" cy=\"" << (1000) << "\" r=\"1\"/>"
       << endl;
    ...
    ss << "<path d=\"M" << (i.x) << "," << (i.y) << " A" << radius << ","
       << radius << " 0 " << largeArcFlag << " " << sweepFlag << " " << (j.x)
       << "," << (j.y)
       << "\" fill=\"none\" stroke=\"green\" stroke-width=\"2\" />" << endl;
